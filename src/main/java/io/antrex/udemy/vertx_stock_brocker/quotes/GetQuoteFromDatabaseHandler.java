...
public class GetQuoteFromDatabaseHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        ...
        SqlTemplate.forQuery(db,
                "SELECT q.asset,q.bid,q.ask,q.last_price,q.volume FROM broker.quotes AS q WHERE asset=#{asset}")
            ...
            .onSuccess(quotes -> {
                if (!quotes.iterator().hasNext()) {
                    DbResponse.notFound(context, "quote for asset "
                        + assetParam + " not available!");
                    return;
                }
                var response = quotes
                    .iterator()
